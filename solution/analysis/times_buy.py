...
if __name__ == '__main__':
    ...
    counter = [len(user.train_label.union(user.test_label)) for user in users]
    for time, count in dict(Counter(counter)).items():
        print time, count, '{:.2f}%'.format(float(count) / len(users) * 100)
