...
if __name__ == '__main__':
    ...
    for user_id, brand_id in view_before:
        ...
        label.append(1 if history[user_id][brand_id]['label'] > 0 else 0)
    ...
    for train_index, test_index in k_fold:
        ...
        n_pos_gen = len(train_data_neg) - n_pos_ori
        ...
        neigh.fit(train_data)
        train_data_pos_clean = []
        for n in xrange(len(train_data_pos)):
            pos = train_data_pos[n]
            _, idx = neigh.kneighbors(pos)
            if train_label[idx[0][0]] > 0: 
                train_data_pos_clean.append(pos)

        train_data_neg_clean = []
        for n in xrange(len(train_data_neg)):
            neg = train_data_neg[n]
            _, idx = neigh.kneighbors(neg)
            if train_label[idx[0][0]] <= 0: 
                train_data_neg_clean.append(neg)

        train_data = train_data_pos_clean + train_data_neg_clean
        train_label = [1 for n in xrange(len(train_data_pos_clean))] \
                    + [0 for n in xrange(len(train_data_neg_clean))]
